    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 260px", gap: 12, height: "100%" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 8, overflow: "auto" }}>
        <div style={{ background: C.s2, border: `1px solid ${C.bd}`, borderRadius: 4, padding: 12 }}>
          <Sec icon="▶" right={<button onClick={run} disabled={drs === "run"} style={{ padding: "3px 12px", fontSize: 9, fontWeight: 600, minWidth: 56, background: drs === "run" ? C.s3 : C.gn, color: drs === "run" ? C.txD : "#000", border: "none", borderRadius: 3, cursor: drs === "run" ? "not-allowed" : "pointer" }}>{drs === "run" ? "실행중..." : drs === "done" ? "↻ 재실행" : "DryRun"}</button>}>Pipeline ({tdd.pipeline.steps.length} steps)</Sec>
